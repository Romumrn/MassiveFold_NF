...
def extract_and_write_to_csv( id_batch, sequence_name, batch_start, batch_end, batch_model):
    """
    ...
    with open(id_batch+"output.csv", mode='w', newline='') as file:
        ...
        for filename in os.listdir("."):
            if filename.endswith(".pickle"):
                ...
                writer.writerow([filename, id_batch, sequence_name, batch_start, batch_end, batch_model, ranking_ptm, ranking_iptm, ranking_debug])
                print(f"Processed {filename}: PTM={ranking_ptm}, IPTM={ranking_iptm}, Debug={ranking_debug}")
...
def main():
    ...
    extract_and_write_to_csv( args.id_batch, args.sequence_name, args.batch_start, args.batch_end, args.batch_model)
